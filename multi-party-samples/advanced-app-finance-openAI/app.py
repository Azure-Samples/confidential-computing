...
@app.route('/')
def index():
    """Serve the main attestation demo page, or the Woodgrove financial dashboard"""
    key_name = os.environ.get('SKR_KEY_NAME', '').lower()
    if 'woodgrove' in key_name:
        return render_template('index-woodgrove.html')
        ...
        if response.status_code != 200:
            ...
            return jsonify({
                ...
                'diagnosis': {
                    ...
                    'command': '.\\Deploy-MultiFinanceAI.ps1 -Build -Deploy'
                },
...
def decrypt_all_fields(record, key, cached_private_key=None):
    ...
    """
    decrypted = {}
    fields = ['transaction_id', 'date_time', 'customer_name', 'customer_age', 'customer_address', 'customer_postal_code', 'customer_country', 'merchant_name', 'merchant_category', 'amount', 'currency']
...
def decrypt_records_batch(records, key, source_name, batch_size=10, cached_private_key=None):
    ...
    Returns:
        List of decrypted records with source tags
    """
...
def partner_analyze():
    """
    Fetch encrypted transaction data from partner containers (Contoso and Fabrikam),
    decrypt ALL fields using the released partner keys, and return financial analytics.
    
    Analytics include (NO PII - no customer names shown):
    - Transaction volume by category
    - Spending patterns by time of day
    - Average monthly mortgage and car loan payments
    ...
    try:
        ...
        # ===== COMPUTE FINANCIAL ANALYTICS (NO PII) =====
        
        # 1. Transaction volume and spend by category
        ...
        for cat, count in category_counts.most_common():
            spend_by_category.append({
                ...
                'total_spend': round(category_spend[cat], 2),
                'avg_transaction': round(category_spend[cat] / count, 2) if count else 0
            })
        
        # 2. Transactions by hour of day (for time-of-day patterns)
        hour_counts = defaultdict(int)
        grocery_hour_counts = defaultdict(int)
        for r in all_records:
            ...
            if dt_str:
                try:
                    ...
                    hour_counts[hour] += 1
                    if r.get('merchant_category') == 'Grocery Store':
                        grocery_hour_counts[hour] += 1
                except (IndexError, ValueError):
                    pass
        
        transactions_by_hour = [{'hour': h, 'count': hour_counts.get(h, 0)} for h in range(24)]
        grocery_by_hour = [{'hour': h, 'count': grocery_hour_counts.get(h, 0)} for h in range(24)]
        peak_grocery_hour = max(grocery_hour_counts, key=grocery_hour_counts.get) if grocery_hour_counts else None
        
        # 3. Average monthly mortgage and car loan payments
        mortgage_amounts = [r['amount'] for r in all_records if r.get('merchant_category') == 'Mortgage' and r.get('amount')]
        car_loan_amounts = [r['amount'] for r in all_records if r.get('merchant_category') == 'Car Loan' and r.get('amount')]
        student_loan_amounts = [r['amount'] for r in all_records if r.get('merchant_category') == 'Student Loan Repayment' and r.get('amount')]
        
        loan_insights = {
            'avg_mortgage_payment': round(sum(mortgage_amounts) / len(mortgage_amounts), 2) if mortgage_amounts else 0,
            'avg_car_loan_payment': round(sum(car_loan_amounts) / len(car_loan_amounts), 2) if car_loan_amounts else 0,
            'avg_student_loan_payment': round(sum(student_loan_amounts) / len(student_loan_amounts), 2) if student_loan_amounts else 0,
            'mortgage_count': len(mortgage_amounts),
            'car_loan_count': len(car_loan_amounts),
            'student_loan_count': len(student_loan_amounts),
        }
        
        # 4. Spending by country (geographic distribution)
        country_spend = defaultdict(lambda: {'total': 0.0, 'count': 0})
        for r in all_records:
            country = r.get('customer_country')
            amt = r.get('amount')
            if country and amt:
                country_spend[country]['total'] += float(amt)
                country_spend[country]['count'] += 1
        
        spending_by_country = []
        for country, data_item in country_spend.items():
            spending_by_country.append({
                'country': country,
                'total_spend': round(data_item['total'], 2),
                'transaction_count': data_item['count'],
                'avg_transaction': round(data_item['total'] / data_item['count'], 2) if data_item['count'] else 0
            })
        spending_by_country.sort(key=lambda x: x['total_spend'], reverse=True)
        
        # 5. Age group spending patterns (aggregated - no individual PII)
        age_groups = {'18-25': [], '26-35': [], '36-45': [], '46-55': [], '56-65': [], '65+': []}
        for r in all_records:
            age = r.get('customer_age')
            amt = r.get('amount')
            if age and amt:
                if age <= 25: age_groups['18-25'].append(float(amt))
                elif age <= 35: age_groups['26-35'].append(float(amt))
                elif age <= 45: age_groups['36-45'].append(float(amt))
                elif age <= 55: age_groups['46-55'].append(float(amt))
                elif age <= 65: age_groups['56-65'].append(float(amt))
                else: age_groups['65+'].append(float(amt))
        
        age_group_insights = []
        for group, amounts in age_groups.items():
            if amounts:
                age_group_insights.append({
                    'age_group': group,
        ...
        # 6. Top merchants by transaction volume (no customer info)
        merchant_counts = Counter(r.get('merchant_name') for r in all_records if r.get('merchant_name'))
        merchant_spend = defaultdict(float)
        for r in all_records:
            if r.get('merchant_name') and r.get('amount'):
                merchant_spend[r['merchant_name']] += float(r['amount'])
        ...
        for merchant, count in merchant_counts.most_common(10):
            top_merchants.append({
                'merchant': merchant,
                'transaction_count': count,
                'total_spend': round(merchant_spend[merchant], 2)
            })
        
        # 7. Spending by partner (Contoso vs Fabrikam)
        contoso_total_spend = sum(float(r['amount']) for r in contoso_records if r.get('amount'))
        fabrikam_total_spend = sum(float(r['amount']) for r in fabrikam_records if r.get('amount'))
        ...
        for r in all_records:
            ...
            if dt_str:
                try:
                    ...
                    d = dt_cls.strptime(dt_str, '%Y-%m-%d %H:%M:%S')
        ...
        return jsonify({
            ...
            'note': 'Financial transaction analytics - no customer PII included'
        })
...
def partner_analyze_stream():
    ...
    def generate():
        ...
        # Phase 4: Compute financial analytics (NO PII)
        yield f"data: {json.dumps({'type': 'status', 'phase': 'analyze', 'message': 'Computing financial analytics...'})}\n\n"
        ...
        # 4. Spending by country
        country_spend_data = defaultdict(lambda: {'total': 0.0, 'count': 0})
        for r in all_records:
            country = r.get('customer_country')
            amt = r.get('amount')
            if country and amt:
                country_spend_data[country]['total'] += float(amt)
                country_spend_data[country]['count'] += 1
        
        spending_by_country = []
        for country, data_item in country_spend_data.items():
            spending_by_country.append({
                'country': country,
                'total_spend': round(data_item['total'], 2),
                'transaction_count': data_item['count'],
...
def partner_demographics_analysis():
    """
    Woodgrove Bank Partner Financial Transaction Analysis.
    Fetches encrypted data from Contoso and Fabrikam containers,
    releases partner keys via SKR, decrypts the data, and returns aggregate financial analytics.
    No customer PII is included in the results.
    """
    import collections
    ...
    try:
        ...
        print(f"\n{'='*60}")
        print(f"[ANALYTICS] Starting Partner Financial Transaction Analysis")
        ...
        for partner_name, partner_url in [('contoso', contoso_url), ('fabrikam', fabrikam_url)]:
            try:
                ...
                for idx, record in enumerate(records):
                    ...
                    # Fields to decrypt
                    encrypted_fields = ['transaction_id', 'date_time', 'customer_name', 'customer_age',
        ...
        if total_records > 0:
            contoso_recs = [r for r in all_decrypted_records if r['company'] == 'contoso']
            fabrikam_recs = [r for r in all_decrypted_records if r['company'] == 'fabrikam']
            results['combined_analytics']['total_records'] = total_records
            results['combined_analytics']['contoso_records'] = len(contoso_recs)
            results['combined_analytics']['fabrikam_records'] = len(fabrikam_recs)
            ...
            for record in all_decrypted_records:
                try:
                    record['customer_age'] = int(record['customer_age'])
            ...
            category_spend = collections.defaultdict(float)
            for r in all_decrypted_records:
                cat = r.get('merchant_category')
                amt = r.get('amount')
                if cat:
                    category_counts[cat] += 1
            ...
            results['combined_analytics']['spend_by_category'] = [
                {'category': cat, 'transaction_count': count,
                 'total_spend': round(category_spend[cat], 2),
                 'avg_transaction': round(category_spend[cat] / count, 2) if count else 0}
                for cat, count in category_counts.most_common()
            ]
            ...
            grocery_hour_counts = collections.defaultdict(int)
            for r in all_decrypted_records:
                dt_str = r.get('date_time')
                if dt_str:
                    try:
            ...
            results['combined_analytics']['transactions_by_hour'] = [
                {'hour': h, 'count': hour_counts.get(h, 0)} for h in range(24)
            ]
            results['combined_analytics']['grocery_by_hour'] = [
                {'hour': h, 'count': grocery_hour_counts.get(h, 0)} for h in range(24)
            ]
            results['combined_analytics']['peak_grocery_hour'] = (
                max(grocery_hour_counts, key=grocery_hour_counts.get) if grocery_hour_counts else None
            )
            
            # 3. Loan insights
            mortgage_amounts = [r['amount'] for r in all_decrypted_records if r.get('merchant_category') == 'Mortgage' and r.get('amount')]
            car_loan_amounts = [r['amount'] for r in all_decrypted_records if r.get('merchant_category') == 'Car Loan' and r.get('amount')]
            student_loan_amounts = [r['amount'] for r in all_decrypted_records if r.get('merchant_category') == 'Student Loan Repayment' and r.get('amount')]
            
            results['combined_analytics']['loan_insights'] = {
                'avg_mortgage_payment': round(sum(mortgage_amounts) / len(mortgage_amounts), 2) if mortgage_amounts else 0,
                'avg_car_loan_payment': round(sum(car_loan_amounts) / len(car_loan_amounts), 2) if car_loan_amounts else 0,
                'avg_student_loan_payment': round(sum(student_loan_amounts) / len(student_loan_amounts), 2) if student_loan_amounts else 0,
                'mortgage_count': len(mortgage_amounts),
            ...
            # 4. Spending by country
            country_spend = collections.defaultdict(lambda: {'total': 0.0, 'count': 0})
            for r in all_decrypted_records:
                country = r.get('customer_country')
                amt = r.get('amount')
                if country and amt:
            ...
            spending_by_country.sort(key=lambda x: x['total_spend'], reverse=True)
            results['combined_analytics']['spending_by_country'] = spending_by_country
            
            # 5. Age group spending patterns
            age_groups = {'18-25': [], '26-35': [], '36-45': [], '46-55': [], '56-65': [], '65+': []}
            for r in all_decrypted_records:
                age = r.get('customer_age')
                amt = r.get('amount')
            ...
            results['combined_analytics']['age_group_insights'] = [
                {'age_group': group, 'transaction_count': len(amounts),
                 'total_spend': round(sum(amounts), 2),
                 'avg_transaction': round(sum(amounts) / len(amounts), 2)}
                for group, amounts in age_groups.items() if amounts
            ]
            
            # 6. Top merchants
            merchant_counts = collections.Counter(r.get('merchant_name') for r in all_decrypted_records if r.get('merchant_name'))
            merchant_spend = collections.defaultdict(float)
            for r in all_decrypted_records:
                if r.get('merchant_name') and r.get('amount'):
                    merchant_spend[r['merchant_name']] += float(r['amount'])
            
            results['combined_analytics']['top_merchants'] = [
                {'merchant': merchant, 'transaction_count': count,
                 'total_spend': round(merchant_spend[merchant], 2)}
                for merchant, count in merchant_counts.most_common(10)
            ]
            
            # 7. Partner spend comparison
            contoso_total_spend = sum(float(r['amount']) for r in contoso_recs if r.get('amount'))
            fabrikam_total_spend = sum(float(r['amount']) for r in fabrikam_recs if r.get('amount'))
            results['combined_analytics']['partner_spend'] = {
                'Contoso': round(contoso_total_spend, 2),
                'Fabrikam': round(fabrikam_total_spend, 2),
                'Combined': round(contoso_total_spend + fabrikam_total_spend, 2),
            }
            ...
            for r in all_decrypted_records:
                ...
                if dt_str:
                    try:
                        ...
                        pass
            results['combined_analytics']['transactions_by_day'] = [
            ...
            results['combined_analytics']['analysis_note'] = (
                ...
                'No customer PII is included in these results.'
            )
            
            print(f"[ANALYTICS] ✓ Financial analytics complete!")
            print(f"[ANALYTICS]   - Total records analyzed: {total_records}")
            print(f"[ANALYTICS]   - Categories found: {len(results['combined_analytics'].get('spend_by_category', []))}")
            print(f"[ANALYTICS]   - Countries found: {len(results['combined_analytics'].get('spending_by_country', []))}")
            print(f"{'='*60}")
            sys.stdout.flush()
        else:
        ...
        return jsonify({
            'status': 'error',
            'message': 'An internal error occurred during partner financial analysis.'
        }), 500
    ...
    try:
        ...
        if not sev_device.get('available', False):
            info['skr_status'] = {
                ...
                'solution': 'Deploy with Confidential SKU: .\\Deploy-MultiFinanceAI.ps1 -Build -Deploy (without -NoAcc)',
...
def populate_from_csv():
    ...
    try:
        ...
        encrypted_records = []
        all_fields = ['transaction_id', 'date_time', 'customer_name', 'customer_age', 'customer_address', 'customer_postal_code', 'customer_country', 'merchant_name', 'merchant_category', 'amount', 'currency']
