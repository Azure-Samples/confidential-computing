...
def vm_security_test():
    ...
    # ---- Test 1: SSH Status ----
    ssh_test = {
        'name': 'Remote shell access to VM via SSH',
    ...
    try:
        ...
        if ssh_status == 'active':
            ...
            ssh_test['detail'] = (
                f'SSH service is {ssh_status}. The setup script disabled SSH at boot time '
                'to eliminate remote shell access as an attack vector. '
                'Even if an operator had the credentials, there is no SSH service listening.'
            )
            ssh_test['policy_rule'] = 'SSH disabled at boot — no remote shell access available'
    except Exception as e:
        ssh_test['result'] = 'unavailable'
        ssh_test['detail'] = f'Unable to check SSH status: {str(e)}'
    tests.append(ssh_test)

    # ---- Test 2: Read unencrypted memory from hypervisor ----
    mem_test = {
        'name': 'Read unencrypted memory from hypervisor',
    ...
    try:
        ...
        if sev_status['available']:
            ...
            mem_test['policy_rule'] = (
                'AMD SEV-SNP encrypts all VM memory with a unique AES key generated by the AMD Secure Processor (SP) '
                'at VM launch. The key never leaves the SP and is not accessible to the hypervisor, host OS, or any '
                'other VM. Even with physical access to the DRAM modules, an attacker sees only ciphertext. SNP adds '
                'integrity protection — any tampering with encrypted memory pages is detected and causes a #VC exception.'
            )
        else:
    ...
    # ---- Test 3: Attach debugger to VM from hypervisor ----
    debug_test = {
        'name': 'Attach debugger to VM from hypervisor',
