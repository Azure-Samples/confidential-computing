...
function Update-KeyReleasePolicy {
    ...
    if ($LASTEXITCODE -eq 0) {
        # Key exists - just update its release policy (no need to delete+recreate)
        Write-Host "  Key already exists. Updating release policy..." -ForegroundColor Yellow
        $updateResult = az keyvault key set-attributes --vault-name $KeyVaultName --name $KeyName --policy $policyPath 2>&1
        if ($LASTEXITCODE -eq 0) {
            Write-Success "  $CompanyName key release policy updated"
        } else {
            Write-Warning "  Failed to update release policy: $($updateResult -join ' ')"
        }
    } else {
        # Key doesn't exist - check if it's in deleted state and needs recovery
        $deletedKey = az keyvault key show-deleted --vault-name $KeyVaultName --name $KeyName 2>&1
        if ($LASTEXITCODE -eq 0) {
            # Key is in deleted state - recover it and update policy
            Write-Host "  Key in deleted state. Recovering..." -ForegroundColor Yellow
            az keyvault key recover --vault-name $KeyVaultName --name $KeyName 2>&1 | Out-Null
            Start-Sleep -Seconds 3
            
            $updateResult = az keyvault key set-attributes --vault-name $KeyVaultName --name $KeyName --policy $policyPath 2>&1
            if ($LASTEXITCODE -eq 0) {
                Write-Success "  $CompanyName key recovered and release policy updated"
            } else {
                Write-Warning "  Failed to update recovered key policy: $($updateResult -join ' ')"
            }
        } else {
            # Key doesn't exist at all - create fresh
            ...
            if ($LASTEXITCODE -eq 0) {
                ...
                Write-Warning "  Key creation failed: $($createResult -join ' ')"
            }
        }
    }
...
function Invoke-Build {
    ...
    $KeyVaultNameD = "kv${RegistryName}d"
    $IdentityNameD = "id-${RegistryName}-wingtip"
    $SkrKeyNameD = "wingtip-secret-key"
    
    Write-Host "Creating 4 Key Vaults and 4 Managed Identities in parallel..." -ForegroundColor Green
    Write-Host "  Key Vaults: $KeyVaultNameA, $KeyVaultNameB, $KeyVaultNameC, $KeyVaultNameD" -ForegroundColor Gray
    Write-Host "  Identities: $IdentityNameA, $IdentityNameB, $IdentityNameC, $IdentityNameD" -ForegroundColor Gray
    Write-Host ""
    ...
    $kvJobD = Start-Job -ScriptBlock { az keyvault create --resource-group $using:ResourceGroup --name $using:KeyVaultNameD --location $using:Location --sku premium --enable-rbac-authorization false 2>&1 }
    ...
    $idJobD = Start-Job -ScriptBlock {
        az rest --method PUT --url "https://management.azure.com/subscriptions/$using:subscriptionId/resourceGroups/$using:ResourceGroup/providers/Microsoft.ManagedIdentity/userAssignedIdentities/$($using:IdentityNameD)?api-version=$using:identityApiVersion" --body "@$using:identityBodyFile" -o json 2>&1
    }

    # Wait for all to complete
    $allJobs = @($kvJobA, $kvJobB, $kvJobC, $kvJobD, $idJobA, $idJobB, $idJobC, $idJobD)
    $null = Wait-Job -Job $allJobs
    
    # Check KV results
    foreach ($j in @($kvJobA, $kvJobB, $kvJobC, $kvJobD)) {
        $result = Receive-Job -Job $j
        if ($j.State -eq 'Failed') { Write-Warning "Key Vault creation had issues: $result" }
    }
    # Check identity results
    foreach ($j in @($idJobA, $idJobB, $idJobC, $idJobD)) {
        $result = Receive-Job -Job $j
    ...
    $idInfoD = az identity show --resource-group $ResourceGroup --name $IdentityNameD -o json 2>$null | ConvertFrom-Json
    ...
    $IdentityClientIdD = $idInfoD.clientId
    ...
    $config = @{
        ...
        # Wingtip Toys configuration
        wingtip = @{
            keyVaultName = $KeyVaultNameD
            skrKeyName = $SkrKeyNameD
            skrAkvEndpoint = "$KeyVaultNameD.vault.azure.net"
            identityName = $IdentityNameD
            identityResourceId = $IdentityResourceIdD
            identityClientId = $IdentityClientIdD
        }
    }
    Save-Config $config
    ...
    Write-Host "  SKR Key: $SkrKeyNameD"
    Write-Host "  Identity: $IdentityNameD"
    Write-Host ""
...
function Invoke-Deploy {
    ...
    Write-Host "  - Wingtip Toys:   Confidential (AMD SEV-SNP TEE) - Toy manufacturer" -ForegroundColor Blue
    ...
    Write-Host "[1/4] Generating Contoso security policy..." -ForegroundColor Cyan
    ...
    Write-Host "[2/4] Generating Fabrikam security policy..." -ForegroundColor Magenta
    ...
    # --- Wingtip Toys Policy Generation ---
    $wingtipConfig = $config.wingtip
    Write-Host "[3/4] Generating Wingtip Toys security policy..." -ForegroundColor Blue
    
    $params_companyD = @{
        '`$schema' = 'https://schema.management.azure.com/schemas/2019-04-01/deploymentParameters.json#'
        'contentVersion' = '1.0.0.0'
        'parameters' = @{
            'containerGroupName' = @{ 'value' = $container_companyD }
            'location' = @{ 'value' = $deployLocation }
            'appImage' = @{ 'value' = $FULL_IMAGE }
            'registryServer' = @{ 'value' = $ACR_LOGIN_SERVER }
            'registryUsername' = @{ 'value' = $ACR_USERNAME }
            'registryPassword' = @{ 'value' = $ACR_PASSWORD }
            'dnsNameLabel' = @{ 'value' = $dns_companyD }
            'skrKeyName' = @{ 'value' = $wingtipConfig.skrKeyName }
            'skrMaaEndpoint' = @{ 'value' = $config.skrMaaEndpoint }
            'skrAkvEndpoint' = @{ 'value' = $wingtipConfig.skrAkvEndpoint }
    ...
    Copy-Item -Path "deployment-template-original.json" -Destination "deployment-template-wingtip.json" -Force
    
    $wingtipPolicyInfo = Get-PolicyHashFromConfcom -TemplatePath "deployment-template-wingtip.json" -ParamsPath "deployment-params-wingtip.json"
    Write-Success "Wingtip policy hash: $($wingtipPolicyInfo.PolicyHash)"
    
    # --- Woodgrove Policy Generation ---
    $woodgroveConfig = $config.woodgrove
    Write-Host "[4/4] Generating Woodgrove-Bank security policy..." -ForegroundColor Yellow
    
    # Build partner container URLs based on DNS names (use config location for correct region)
    # Use HTTP for inter-container communication - data is already encrypted with SKR keys
    # HTTPS with self-signed certs causes RemoteDisconnected errors between ACI containers
    $contosoContainerUrl = "http://${dns_companyA}.${deployLocation}.azurecontainer.io"
    $fabrikamContainerUrl = "http://${dns_companyB}.${deployLocation}.azurecontainer.io"
    $wingtipContainerUrl = "http://${dns_companyD}.${deployLocation}.azurecontainer.io"
    ...
    Write-Host "║  Wingtip:   $($wingtipPolicyInfo.PolicyHash)  ║" -ForegroundColor Blue
    ...
    # Check if key exists - update policy if so, recover if deleted, create if new
    $existingKey = az keyvault key show --vault-name $contosoConfig.keyVaultName --name $contosoConfig.skrKeyName 2>&1
    if ($LASTEXITCODE -eq 0) {
        Write-Host "    Contoso key exists. Updating release policy..." -ForegroundColor Yellow
        az keyvault key set-attributes --vault-name $contosoConfig.keyVaultName --name $contosoConfig.skrKeyName --policy $contosoMultiPolicyPath 2>&1 | Out-Null
        if ($LASTEXITCODE -eq 0) { Write-Success "  Contoso key release policy updated (allows Contoso + Woodgrove)" }
        else { Write-Warning "  Failed to update Contoso key release policy" }
    } else {
        $deletedKey = az keyvault key show-deleted --vault-name $contosoConfig.keyVaultName --name $contosoConfig.skrKeyName 2>&1
        if ($LASTEXITCODE -eq 0) {
            Write-Host "    Contoso key in deleted state. Recovering..." -ForegroundColor Yellow
            az keyvault key recover --vault-name $contosoConfig.keyVaultName --name $contosoConfig.skrKeyName 2>&1 | Out-Null
            Start-Sleep -Seconds 3
            az keyvault key set-attributes --vault-name $contosoConfig.keyVaultName --name $contosoConfig.skrKeyName --policy $contosoMultiPolicyPath 2>&1 | Out-Null
            if ($LASTEXITCODE -eq 0) { Write-Success "  Contoso key recovered and policy updated (allows Contoso + Woodgrove)" }
            else { Write-Warning "  Failed to update recovered Contoso key policy" }
        } else {
            ...
            if ($LASTEXITCODE -eq 0) { Write-Success "  Contoso key created: allows Contoso + Woodgrove containers" }
            else { Write-Warning "  Contoso key creation failed" }
        }
    }
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Host "    Fabrikam key exists. Updating release policy..." -ForegroundColor Yellow
        az keyvault key set-attributes --vault-name $fabrikamConfig.keyVaultName --name $fabrikamConfig.skrKeyName --policy $fabrikamMultiPolicyPath 2>&1 | Out-Null
        if ($LASTEXITCODE -eq 0) { Write-Success "  Fabrikam key release policy updated (allows Fabrikam + Woodgrove)" }
        else { Write-Warning "  Failed to update Fabrikam key release policy" }
    } else {
        $deletedKey = az keyvault key show-deleted --vault-name $fabrikamConfig.keyVaultName --name $fabrikamConfig.skrKeyName 2>&1
        if ($LASTEXITCODE -eq 0) {
            Write-Host "    Fabrikam key in deleted state. Recovering..." -ForegroundColor Yellow
            az keyvault key recover --vault-name $fabrikamConfig.keyVaultName --name $fabrikamConfig.skrKeyName 2>&1 | Out-Null
            Start-Sleep -Seconds 3
            az keyvault key set-attributes --vault-name $fabrikamConfig.keyVaultName --name $fabrikamConfig.skrKeyName --policy $fabrikamMultiPolicyPath 2>&1 | Out-Null
            if ($LASTEXITCODE -eq 0) { Write-Success "  Fabrikam key recovered and policy updated (allows Fabrikam + Woodgrove)" }
            else { Write-Warning "  Failed to update recovered Fabrikam key policy" }
        } else {
            ...
            if ($LASTEXITCODE -eq 0) { Write-Success "  Fabrikam key created: allows Fabrikam + Woodgrove containers" }
            else { Write-Warning "  Fabrikam key creation failed" }
        }
    }
    Remove-Item $fabrikamMultiPolicyPath -Force -ErrorAction SilentlyContinue
    
    # --- Create Wingtip Key (allows Wingtip + Woodgrove) ---
    Write-Host "Creating Wingtip key (multi-party: Wingtip + Woodgrove)..." -ForegroundColor Blue
    $wingtipMultiPartyPolicy = @{
        version = "1.0.0"
        anyOf = @(
            @{
                authority = "https://$($config.skrMaaEndpoint)"
                allOf = @(
                    @{ claim = "x-ms-attestation-type"; equals = "sevsnpvm" },
                    @{ claim = "x-ms-sevsnpvm-hostdata"; equals = $wingtipPolicyInfo.PolicyHash }
                )
            },
            @{
                authority = "https://$($config.skrMaaEndpoint)"
    ...
    $wingtipMultiPolicyPath = Join-Path $PSScriptRoot "release-policy-wingtip-multiparty.json"
    $wingtipMultiPolicyJson = $wingtipMultiPartyPolicy | ConvertTo-Json -Depth 10
    $wingtipMultiPolicyJson | Out-File -FilePath $wingtipMultiPolicyPath -Encoding UTF8
    
    # Display Wingtip release policy on the console
    Write-Host ""
    Write-Host "╔══════════════════════════════════════════════════════════════════════════════╗" -ForegroundColor Blue
    Write-Host "║  WINGTIP KEY RELEASE POLICY (Multi-Party: Wingtip + Woodgrove)               ║" -ForegroundColor Blue
    Write-Host "╠══════════════════════════════════════════════════════════════════════════════╣" -ForegroundColor Blue
    $wingtipMultiPolicyJson -split "`n" | ForEach-Object { Write-Host "  $_" -ForegroundColor Gray }
    Write-Host "╚══════════════════════════════════════════════════════════════════════════════╝" -ForegroundColor Blue
    Write-Host ""
    
    # Check if key exists - update policy if so, recover if deleted, create if new
    $existingKey = az keyvault key show --vault-name $wingtipConfig.keyVaultName --name $wingtipConfig.skrKeyName 2>&1
    if ($LASTEXITCODE -eq 0) {
        Write-Host "    Wingtip key exists. Updating release policy..." -ForegroundColor Yellow
        az keyvault key set-attributes --vault-name $wingtipConfig.keyVaultName --name $wingtipConfig.skrKeyName --policy $wingtipMultiPolicyPath 2>&1 | Out-Null
        if ($LASTEXITCODE -eq 0) { Write-Success "  Wingtip key release policy updated (allows Wingtip + Woodgrove)" }
        else { Write-Warning "  Failed to update Wingtip key release policy" }
    } else {
        $deletedKey = az keyvault key show-deleted --vault-name $wingtipConfig.keyVaultName --name $wingtipConfig.skrKeyName 2>&1
        if ($LASTEXITCODE -eq 0) {
            Write-Host "    Wingtip key in deleted state. Recovering..." -ForegroundColor Yellow
            az keyvault key recover --vault-name $wingtipConfig.keyVaultName --name $wingtipConfig.skrKeyName 2>&1 | Out-Null
            Start-Sleep -Seconds 3
            az keyvault key set-attributes --vault-name $wingtipConfig.keyVaultName --name $wingtipConfig.skrKeyName --policy $wingtipMultiPolicyPath 2>&1 | Out-Null
            if ($LASTEXITCODE -eq 0) { Write-Success "  Wingtip key recovered and policy updated (allows Wingtip + Woodgrove)" }
            else { Write-Warning "  Failed to update recovered Wingtip key policy" }
        } else {
            az keyvault key create --vault-name $wingtipConfig.keyVaultName --name $wingtipConfig.skrKeyName `
                --kty RSA-HSM --size 2048 --ops wrapKey unwrapKey encrypt decrypt --exportable true `
                --policy $wingtipMultiPolicyPath 2>&1 | Out-Null
            if ($LASTEXITCODE -eq 0) { Write-Success "  Wingtip key created: allows Wingtip + Woodgrove containers" }
            else { Write-Warning "  Wingtip key creation failed" }
        }
    }
    Remove-Item $wingtipMultiPolicyPath -Force -ErrorAction SilentlyContinue
    ...
    Write-Host "║  Wingtip Key:   Releases to Wingtip OR Woodgrove containers                  ║" -ForegroundColor White
    ...
    $params_companyD.parameters['securityPolicyHash'] = @{ 'value' = $wingtipPolicyInfo.PolicyHash }
    $params_companyD | ConvertTo-Json -Depth 10 | Set-Content 'deployment-params-wingtip.json'
    ...
    Write-Host "Deploying all 4 containers in parallel..." -ForegroundColor Cyan
    ...
    $deployJobD = Start-Job -ScriptBlock {
        Set-Location $using:scriptRoot
        az deployment group create --resource-group $using:resource_group --template-file deployment-template-wingtip.json --parameters '@deployment-params-wingtip.json' 2>&1
    }
    ...
    $deployJobs = @(
        ...
        @{ Job = $deployJobD; Name = "Wingtip Toys"; Color = "Blue" },
    ...
    Remove-Job -Job @($deployJobA, $deployJobB, $deployJobC, $deployJobD) -Force
    Write-Success "All 4 containers deployed!"
    ...
    $fqdn_companyD = az container show --resource-group $resource_group --name $container_companyD --query "ipAddress.fqdn" --output tsv
    
    Write-Host "FQDNs:"
    Write-Host "  Contoso:        http://$fqdn_companyA" -ForegroundColor Green
    Write-Host "  Fabrikam:       http://$fqdn_companyB" -ForegroundColor Green
    Write-Host "  Wingtip Toys:   http://$fqdn_companyD" -ForegroundColor Green
    ...
    $companyD_ready = $false
    
    while ($elapsed_seconds -lt $timeout_seconds -and (-not $companyA_ready -or -not $companyB_ready -or -not $companyC_ready -or -not $companyD_ready)) {
        ...
        if (-not $companyD_ready) {
            try {
                $response = Invoke-WebRequest -Uri "http://$fqdn_companyD" -Method Head -TimeoutSec 5 -ErrorAction SilentlyContinue
                if ($response.StatusCode -eq 200) { 
                    $companyD_ready = $true
                    Write-Success "Wingtip Toys is ready!"
                }
            } catch { }
        }
        ...
        if (-not $companyA_ready -or -not $companyB_ready -or -not $companyC_ready -or -not $companyD_ready) {
            ...
            $status += "Wingtip: $(if ($companyD_ready) { 'Ready' } else { '...' }), "
    ...
    if (-not $companyA_ready -or -not $companyB_ready -or -not $companyC_ready -or -not $companyD_ready) {
        ...
        if (-not $companyD_ready) { Write-Warning "  - Wingtip Toys not ready" }
    ...
    if (-not $SkipBrowser) {
        ...
        Write-Host "  Tab 3: Wingtip Toys - $urlD"
        Write-Host "  Tab 4: Woodgrove    - $urlC"
        Write-Host ""
        # Open Edge with all four URLs as separate tabs in a new window
        $edgeProcess = Start-Process "msedge" -ArgumentList "--new-window `"$urlA`" `"$urlB`" `"$urlD`" `"$urlC`"" -PassThru
    } else {
    ...
    az container delete --resource-group $resource_group --name $container_companyD --yes 2>&1 | Out-Null
    ...
    Remove-Item -Path "deployment-params-wingtip.json" -Force -ErrorAction SilentlyContinue
    ...
    Remove-Item -Path "deployment-template-wingtip.json" -Force -ErrorAction SilentlyContinue
...
function Invoke-DeployAKS {
    ...
    Write-Host "  - Wingtip-Toys:   Confidential Virtual Node Pod (AMD SEV-SNP TEE)" -ForegroundColor Cyan
    ...
    $pod_companyD = "wingtip-$timestamp"
    $dns_companyA = "contoso-$timestamp"
    $dns_companyB = "fabrikam-$timestamp"
    $dns_companyC = "woodgrove-$timestamp"
    $dns_companyD = "wingtip-$timestamp"
    ...
    Write-Host "  Wingtip-Toys:   $pod_companyD"
    ...
    # --- Contoso Pod YAML ---
    Write-Host "[1/4] Generating Contoso pod YAML..." -ForegroundColor Cyan
    ...
    # --- Fabrikam Pod YAML ---
    Write-Host "[2/4] Generating Fabrikam pod YAML..." -ForegroundColor Magenta
    ...
    # --- Wingtip Toys Pod YAML ---
    Write-Host "[3/4] Generating Wingtip Toys pod YAML..." -ForegroundColor Cyan
    $wingtipYaml = New-VirtualNodePodYaml `
        -PodName $pod_companyD `
        -CompanyLabel "wingtip" `
        -Image $FULL_IMAGE `
        -IdentityResourceId $wingtipConfig.identityResourceId `
        -DnsLabel $dns_companyD `
        -SkrKeyName $wingtipConfig.skrKeyName `
        -SkrMaaEndpoint $config.skrMaaEndpoint `
        -SkrAkvEndpoint $wingtipConfig.skrAkvEndpoint `
        -StorageConnectionString $StorageConnectionString `
        -ResourceGroupName $resource_group `
        -AciSubnetName $aciSubnetName
    $wingtipYamlPath = Join-Path $PSScriptRoot "pod-wingtip.yaml"
    $wingtipYaml | Set-Content $wingtipYamlPath -Encoding UTF8
    
    $wingtipPolicyInfo = Get-PolicyHashFromVirtualNodeYaml -YamlPath $wingtipYamlPath
    Write-Success "Wingtip Toys policy hash: $($wingtipPolicyInfo.PolicyHash)"
    
    # NOTE: Woodgrove pod YAML is generated AFTER Contoso, Fabrikam and Wingtip pods are deployed
    ...
    Write-Host ""
    Write-Host "╔══════════════════════════════════════════════════════════════════════════════╗" -ForegroundColor Cyan
    Write-Host "║  SECURITY POLICY HASHES - CONTOSO, FABRIKAM & WINGTIP (AKS Virtual Nodes)    ║" -ForegroundColor Cyan
    Write-Host "╠══════════════════════════════════════════════════════════════════════════════╣" -ForegroundColor Cyan
    Write-Host "║  Contoso:      $($contosoPolicyInfo.PolicyHash)  ║" -ForegroundColor Green
    Write-Host "║  Fabrikam:     $($fabrikamPolicyInfo.PolicyHash)  ║" -ForegroundColor Magenta
    Write-Host "║  Wingtip Toys: $($wingtipPolicyInfo.PolicyHash)  ║" -ForegroundColor Cyan
    ...
    Write-Header "Phase 2: Deploying Contoso, Fabrikam & Wingtip Pods"
    Write-Host "Deploying Contoso, Fabrikam and Wingtip first to get pod IPs for Woodgrove partner URLs." -ForegroundColor Yellow
    Write-Host ""
    ...
    Write-Host "  Deploying Wingtip Toys pod..." -ForegroundColor Cyan
    kubectl apply -f $wingtipYamlPath 2>&1
    if ($LASTEXITCODE -ne 0) { Write-Warning "Wingtip pod apply returned non-zero" }
    
    Write-Host ""
    Write-Host "Waiting for Contoso, Fabrikam and Wingtip pods to start (may take 2-5 minutes)..." -ForegroundColor Gray
    ...
    while ($elapsed_seconds -lt $timeout_seconds -and (-not $companyA_ready -or -not $companyB_ready -or -not $companyD_ready)) {
        try {
            ...
            if ($pods -and $pods.items) {
                foreach ($pod in $pods.items) {
                    ...
                    if ($podName -eq $pod_companyD -and $phase -eq "Running" -and -not $companyD_ready) {
                        $companyD_ready = $true
                        Write-Success "Wingtip Toys pod is Running!"
                    }
                }
            }
        } catch {
        ...
        if (-not $companyA_ready -or -not $companyB_ready -or -not $companyD_ready) {
            $status = "Waiting... ($elapsed_seconds/$timeout_seconds sec) - "
            $status += "Contoso: $(if ($companyA_ready) { 'Running' } else { '...' }), "
            $status += "Fabrikam: $(if ($companyB_ready) { 'Running' } else { '...' }), "
            $status += "Wingtip: $(if ($companyD_ready) { 'Running' } else { '...' })"
            Write-Host $status
    ...
    if (-not $companyA_ready -or -not $companyB_ready -or -not $companyD_ready) {
        Write-Warning "Contoso/Fabrikam/Wingtip pods did not start in time."
    ...
    $podIpD = (kubectl get pod $pod_companyD -o jsonpath='{.status.podIP}' 2>$null)
    ...
    Write-Host "  Wingtip Toys: $podIpD" -ForegroundColor Gray
    Write-Host ""
    ...
    $woodgroveExtraEnv = @{
        ...
        'PARTNER_WINGTIP_AKV_ENDPOINT' = $config.wingtip.skrAkvEndpoint
        'PARTNER_CONTOSO_URL' = "http://$podIpA"
        'PARTNER_FABRIKAM_URL' = "http://$podIpB"
        'PARTNER_WINGTIP_URL' = "http://$podIpD"
    }
    ...
    # --- Create Wingtip Toys Key (allows Wingtip + Woodgrove) ---
    Write-Host "Creating Wingtip Toys key (multi-party: Wingtip + Woodgrove)..." -ForegroundColor Cyan
    $wingtipMultiPartyPolicy = @{
        version = "1.0.0"
        anyOf = @(
            @{
    ...
    $wingtipMultiPolicyPath = Join-Path $PSScriptRoot "release-policy-wingtip-multiparty.json"
    $wingtipMultiPartyPolicy | ConvertTo-Json -Depth 10 | Out-File -FilePath $wingtipMultiPolicyPath -Encoding UTF8
    
    Write-Host ""
    Write-Host "╔══════════════════════════════════════════════════════════════════════════════╗" -ForegroundColor Cyan
    Write-Host "║  WINGTIP KEY RELEASE POLICY (Multi-Party: Wingtip + Woodgrove)                ║" -ForegroundColor Cyan
    Write-Host "╠══════════════════════════════════════════════════════════════════════════════╣" -ForegroundColor Cyan
    ($wingtipMultiPartyPolicy | ConvertTo-Json -Depth 10) -split "`n" | ForEach-Object { Write-Host "  $_" -ForegroundColor Gray }
    Write-Host "╚══════════════════════════════════════════════════════════════════════════════╝" -ForegroundColor Cyan
    Write-Host ""
    ...
    Write-Host "║  Wingtip Key:      Releases to Wingtip OR Woodgrove pods                    ║" -ForegroundColor White
...
"@
    ...
    Write-Host "  Wingtip-Toys:   $urlD" -ForegroundColor Green
    ...
    $httpTimeout = 60
    $httpElapsed = 0
    $httpA = $false; $httpB = $false; $httpC = $false; $httpD = $false

    while ($httpElapsed -lt $httpTimeout -and (-not $httpA -or -not $httpB -or -not $httpC -or -not $httpD)) {
        if (-not $httpA) {
            try {
        ...
        if (-not $httpD) {
            try {
                $response = Invoke-WebRequest -Uri $urlD -Method Head -TimeoutSec 5 -UseBasicParsing -ErrorAction SilentlyContinue
                if ($response.StatusCode -eq 200) { $httpD = $true; Write-Success "Wingtip Toys HTTP ready!" }
            } catch { }
        }

        if (-not $httpA -or -not $httpB -or -not $httpC -or -not $httpD) {
            [System.Threading.Thread]::Sleep(5000)
            $httpElapsed += 5
        }
    }
    ...
    kubectl delete pod $pod_companyD --ignore-not-found 2>&1 | Out-Null
    ...
    Remove-Item -Path $wingtipYamlPath -Force -ErrorAction SilentlyContinue
